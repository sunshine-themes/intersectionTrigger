...
class Animation {
	...
	killed: boolean;
	seekSmoothly: (ins: any, seekTo: any, link: any, isSeekToGreater: any) => void;
	seek: (ins: any, seekTo: any, link: any) => void;
	startSnaping: ({
		snapDistance,
		currentDis,
		snap,
	...
	setUtils() {
		...
		this.parseSnap = ({ instance, snap }: { instance: Anime<AnimeInstance>; snap: SnapConfiguration | SnapParams }, update?: boolean) => {
			...
			const mergeOpts = (customOpts: SnapDefaultConfiguration) => mergeOptions(snapDefaultConfig, customOpts);
			const parseOriginalToParam = (to: number | string | number[]) => (is.num(to) ? parseNum(to) : parseMarks());

			let snapParams = {} as SnapParams;
			let mergedParams = {} as DeepRequired<SnapDefaultConfiguration>;
			...
			if (update) {
				...
				if (is.object(snap)) mergedParams = mergeOpts(snap as SnapDefaultConfiguration);
	...
	parse(params: Anime<AnimeInstance> | DefaultAnimationConfiguration): AnimationParams;
	parse(params: AnimationParams, update: boolean): AnimationParams;
	parse(params: Anime<AnimeInstance> | DefaultAnimationConfiguration | AnimationParams, update?: boolean) {
		let mergedParams = {} as DeepRequired<DefaultAnimationConfiguration>,
