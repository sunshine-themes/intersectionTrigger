    ...
    <header className="h-[var(--nav-height)] sticky top-0 z-30 border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="flex items-center justify-between h-full px-4 lg:px-6">
        <div className="flex items-center gap-2.5">
          <img src={logo} alt="IntersectionTrigger" className="h-7 w-7" />
          <span className="font-bold text-foreground text-sm tracking-tight">
            IntersectionTrigger
          </span>
          <span className="hidden sm:inline-flex text-xs px-1.5 py-0.5 rounded bg-accent text-accent-foreground font-mono">
            v1.1.6
          </span>
        </div>
