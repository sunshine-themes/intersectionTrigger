...
var IntersectionTrigger = class {
  ...
  _observerCallback = (entries, observer) => {
    ...
    for (const entry of entries) {
      ...
      const rB = this.rootBounds, rL = rB[length], rootToTarget = rL / tB[length];
      ...
      const initBackupFun = tB[length] >= rL && (minPosition >= rootToTarget || 1 - maxPosition >= rootToTarget), isBackupFunRunning = !!backup;
      switch (true) {
