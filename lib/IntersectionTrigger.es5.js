    ...
    _createClass(Utils2, [{
      ...
      value: function setUtils() {
        ...
        this.setRootMargin = function() {
          var extendMargin = getScrollValue(_this.isRootViewport() ? document.body : _this._it._root, _this.isVirtical() ? "x" : "y");
          return _this.isVirtical() ? "".concat(_this._it._positions.rootLeavePosition.strValue, " ").concat(extendMargin, "px ").concat(_this._it._positions.rootEnterPosition.strValue, " ").concat(extendMargin, "px") : "".concat(extendMargin, "px ").concat(_this._it._positions.rootEnterPosition.strValue, " ").concat(extendMargin, "px ").concat(_this._it._positions.rootLeavePosition.strValue);
        };
        this.setThreshold = function() {
          var threshold = [0, _this._it._triggerParams.enter, _this._it._triggerParams.leave, roundFloat(1 - _this._it._triggerParams.leave, 2), 1];
        ...
        this.validatePosition = function(name, pos) {
          ...
          if (!is.string(pos))
            throwError("".concat(name, " parameter must be a string."));
          return pos;
        };
        this.setPositionData = function(offset) {
          ...
          var isPercentage = is.percent(value);
          var isPixel = is.pixel(value);
          var position = {};
          ...
          var _this$dirProps = _this.dirProps(), length = _this$dirProps.length, innerLength = _this$dirProps.innerLength;
          var rootLength = _this._it._root ? getBoundsProp(_this._it._root, length) : innerLength;
          switch (true) {
        ...
        this.parsePositions = function() {
          ...
          return {
            triggerEnterPosition: _this.setPositionData(positions[0]),
            rootEnterPosition: _this.setPositionData(positions[1], false),
            triggerLeavePosition: _this.setPositionData(positions[2]),
            rootLeavePosition: _this.setPositionData(positions[3], false, false)
          };
        };
        ...
        this.onTriggerEnter = function(trigger) {
          ...
          toggleClass && _this.toggleClass(trigger, toggleClass, data.eventIndex);
          animation && _this.animate(trigger, animation, data.eventIndex);
        ...
        this.animate = function(trigger, animation, eventIndex) {
          var instance = animation.instance, toggleActions = animation.toggleActions, control = animation.control, snap = animation.snap;
          if (!instance)
            return;
          if (control) {
            var _this$getTriggerState3 = _this.getTriggerStates(trigger, "onScroll"), animate = _this$getTriggerState3.animate;
            var ids = _this.getTriggerStates(trigger, "ids");
            ...
            var isVir = _this.isVirtical();
            var tB = trigger.getBoundingClientRect();
            var tIL = tB[length] - (enter * tB[length] + (1 - leave) * tB[length]);
            var root = _this.getRoot();
            var duration = instance.duration;
            var animateHandler = function animateHandler2(trigger2) {
              var tB2 = trigger2.getBoundingClientRect();
              var ids2 = _this.getTriggerStates(trigger2, "ids");
              _this._it.rootBounds = _this.getRootRect(_this._it.observer.rootMargin);
              var rB = _this._it.rootBounds;
              var scrollLength = tIL + rB[length];
              var currentTime = 0;
              var diff = rB[refOpposite] - (tB2[ref] + enter * tB2[length]);
              if (diff > 0) {
                currentTime = duration * diff / scrollLength;
                if (is.num(control)) {
                  setTimeout(function() {
                    return instance.seek(currentTime);
                  }, control * 1e3);
                } else {
              ...
              if (snap) {
                var speed = snap.speed * 17 / 1e3;
                var dis = 0;
                var startSnaping = function startSnaping2(snapDistance) {
                  var toRef = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : false;
                  ...
                  requestAnimationFrame(function() {
                    return startSnaping2(snapDistance, toRef);
                  });
                };
                ...
                var snapTimeOutId = setTimeout(function() {
                  ...
                  var diff2 = directionalDiff.map(function(n) {
                    return Math.abs(n);
                  });
                  var closest = Math.min.apply(Math, _toConsumableArray(diff2));
                  var closestWithDirection = directionalDiff[diff2.indexOf(closest)];
                  var snapDistance = scrollLength * closest / duration;
                ...
                _this.setTriggerStates(trigger2, {
                  ids: _objectSpread(_objectSpread({}, ids2), {}, {
            ...
            switch (eventIndex) {
              case 0:
              case 2:
                _this._it._states.oCbFirstInvoke && animateHandler(trigger);
                if (animate)
                  break;
                _this.setTriggerScrollStates(trigger, "animate", animateHandler);
                break;
              case 1:
              case 3:
                clearTimeout(ids.snapTimeOutId);
                _this.setTriggerScrollStates(trigger, "animate", null);
                break;
            }
            return;
          }
        ...
        this.parseAnimation = function(params) {
          var animation = {};
          switch (true) {
            ...
            case is.object(params):
              {
                var mergedParams = mergeOptions(defaultAnimationParams, params);
                var instance = mergedParams.instance, toggleActions = mergedParams.toggleActions, snap = mergedParams.snap;
        ...
        this.toggleClass = function(trigger, toggleClass, eventIndex) {
        ...
        this.parseToggleClass = function(params) {
          var toggleClass = [];
          if (is.string(params)) {
          ...
          if (is.array(params)) {
            toggleClass = params.map(function(obj) {
              ...
              targets && (mergedParams2.targets = _this.parseQuery(targets, "targets"));
        ...
        this.toggleActions = function(trigger) {
          ...
          _this._it.rootBounds = _this.getRootRect(_this._it.observer.rootMargin);
          var rB = _this._it.rootBounds;
          var _this$getTriggerData6 = _this.getTriggerData(trigger), enter = _this$getTriggerData6.enter, leave = _this$getTriggerData6.leave;
          var _this$getTriggerState4 = _this.getTriggerStates(trigger), hasEnteredFromOneSide = _this$getTriggerState4.hasEnteredFromOneSide, hasLeft = _this$getTriggerState4.hasLeft, hasLeftBack = _this$getTriggerState4.hasLeftBack;
          var _this$dirProps3 = _this.dirProps(), ref = _this$dirProps3.ref, refOpposite = _this$dirProps3.refOpposite, length = _this$dirProps3.length;
          var hasCaseMet = true;
          switch (true) {
        ...
        this.parseString = function(string2) {
          var parsedString = string2.split(/\s+/).map(function(margin) {
        ...
        this.getRootRect = function(rootMargins) {
          ...
          if (_this._it._root && !is.doc(_this._it._root)) {
            rootRect = boundsMinusScrollbar(_this._it._root);
  ...
  var IntersectionTrigger = /* @__PURE__ */ function() {
      ...
      _defineProperty2(this, "_observerCallback", function(entries, observer) {
        ...
        try {
          for (_iterator.s(); !(_step = _iterator.n()).done; ) {
            ...
            var _this$_utils$getTrigg = _this._utils.getTriggerData(trigger), enter = _this$_utils$getTrigg.enter, leave = _this$_utils$getTrigg.leave;
            ...
            var isTriggerLarger = tB[length] >= rootLength, isTSPLarger = enter > rootToTarget, isTEPLarger = 1 - leave > rootToTarget, initBackupFun = isTriggerLarger && (isTSPLarger || isTEPLarger), isBackupFunRunning = !!backup;
    ...
    _createClass2(IntersectionTrigger2, [{
      ...
      value: function _setInstance() {
        ...
        var _this$_options$defaul = this._options.defaults, once = _this$_options$defaul.once, onEnter = _this$_options$defaul.onEnter, onLeave = _this$_options$defaul.onLeave, onEnterBack = _this$_options$defaul.onEnterBack, onLeaveBack = _this$_options$defaul.onLeaveBack, toggleClass = _this$_options$defaul.toggleClass, animation = _this$_options$defaul.animation;
        this._triggerParams = {
      ...
      value: function add() {
        ...
        var triggerParams = _objectSpread2(_objectSpread2({}, mergeOptions(this._triggerParams, options)), {}, {
        ...
        [options.enter, options.leave].forEach(function(position) {
          return !_this2._threshold.some(function(value) {
        ...
        if (mustRecreateObserver) {
          ...
          toAddTriggers.forEach(function(trigger2) {
            return _this2._utils.setTriggerData(trigger2, triggerParams);
          });
          this._createInstance();
          this.triggers.forEach(function(trigger2) {
            return _this2.observer.observe(trigger2);
          });
        } else {
          toAddTriggers.forEach(function(trigger2) {
            _this2._utils.setTriggerData(trigger2, triggerParams);
            _this2.observer.observe(trigger2);
          });
        }
      ...
      value: function remove() {
        ...
        toRemoveTriggers.forEach(function(trigger2) {
          _this3._utils.deleteTriggerData(trigger2);
          _this3.observer.unobserve(trigger2);
        });
